...
public class InterDB {
    ...
    public static void insertVao(Connection con, Object obj) throws Exception {
        ...
        HashMap<String, Method> map = InterDB.fieldsGetter(c, obj);
        if (map.isEmpty()) {
            throw new NoReference("Empty object");
        }
        ...
        try {
            ...
            int status = stmt.executeUpdate();
            if(meta.getIdType()==IdType.SERIAL){
                getSerialId(con, meta.getTable(), obj, map, fieldId);
            }
        } catch (Exception e) {
            e.printStackTrace();
            throw e;
            ...
            for (int i = 0; i < varKeys.length; i++) {
                if (i == 0) {
                    ...
                    for (int ii = 0; ii < var.get(varKeys[i]).size(); ii++) {
                        ...
                        } else if (var.get(varKeys[i]).get(ii) == null) {
...
//                    indice++;
    ...
    private static HashMap<String, Method> fieldsGetter(Class c, Object obj) throws Exception {
        HashMap<String, Method> map = null;
        Class sup = c.getSuperclass();
        if (sup != null && !sup.equals(Object.class)) {
            map = fieldsGetter(sup, obj);
        } else {
            map = new HashMap<String, Method>();
        }
        
        Set<FieldDAO> fdaos = MAPPING_METADATA.get(c.getCanonicalName()).getFieldsMetaData();
        for (FieldDAO fdao : fdaos) {
            if (fdao.getNumRef() != null) {
                boolean isset = (boolean) fdao.getNumRefGetter().invoke(obj);
                if (isset) {
    ...
    private static MappingMetaData mapClass(Class c,Connection con) throws Exception{
        MappingMetaData meta= new MappingMetaData(c, con);
